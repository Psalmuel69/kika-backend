...
- When several sales in the SAME message have no item/customer name at all but a payment method IS stated for each ("₦45,000 transfer, ₦12,000 POS, and ₦8,000 cash"), the payment method is real, distinguishing information — use it ("Sale via transfer", "Sale via POS", "Sale via cash"). Never describe two or more genuinely different transactions with the identical bare word "Sale" (or any other identical generic label) when ANYTHING in the message could tell them apart — a receipt with three indistinguishable "Sale" lines is barely better than not recording the distinction at all.
- Only fall back to a fully generic label ("Sale", "Expense") when truly nothing else in the message — no item, no name, no payment method, no purpose — differentiates that transaction from another one nearby.
