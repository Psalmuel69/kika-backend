...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (link.status === 'EXPIRED' || (link.status === 'PENDING' && new Date(link.expires_at) < new Date())) {
      ...
      return res.status(410).send('<h1>This payment link has expired</h1><p>Please request a new one.</p>');
    }
    if (link.status === 'PAID') {
      return res.status(200).send('<h1>This payment has already been completed</h1><p>Thank you!</p>');
    }
