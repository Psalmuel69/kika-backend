...
--
-- This is the entire schema as it existed before Kika adopted versioned,
-- tracked migrations (see src/db/migrate.js and README.md in this
-- directory) — captured as-is as the starting point, rather than
-- retroactively split into a "fake" granular history that would just be
-- guessing at where each past change actually landed.
--
-- Every statement below is written to be safely re-runnable (CREATE
-- TABLE IF NOT EXISTS, ADD COLUMN IF NOT EXISTS, DROP CONSTRAINT IF
-- EXISTS + re-ADD, etc.) — this was already true of the schema before
-- this migration system existed, and that property is exactly what
-- makes it safe to run this file as migration 0001 against ANY existing
-- database, however far along its own history already is: on a fresh
-- database it creates everything from scratch; on a database that
-- already has some or all of this applied (from the old
-- run-the-whole-file-every-time approach), every statement is a no-op
-- for what's already there and only applies what's missing. Either way,
-- once this migration is recorded as applied (see the
-- schema_migrations tracking table in migrate.js), it never runs again.
--
-- ALL FUTURE schema changes go in NEW numbered files in this directory
-- (0002_*.sql, 0003_*.sql, ...), each a focused, reviewable diff of ONE
-- change — never by editing this file or appending more to the bottom
...
CREATE TABLE IF NOT EXISTS merchants (
    ...
    invoice_awaiting_stage VARCHAR(20) CHECK (invoice_awaiting_stage IN ('AWAITING_NAME', 'AWAITING_PHONE', 'ITEMS', 'CONFIRM')),
...
-- ---------------------------------------------------------------------------
-- Migration: the invoice "ask for a phone number before collecting
-- items" step.
-- ---------------------------------------------------------------------------

-- AWAITING_PHONE: a new invoice-flow stage — "new invoice for <name>"
-- with no phone attached now asks for the customer's number (so it can
-- be saved against the customer record and shown on the invoice's
-- "Billed to" block) before moving on to item collection. A no-op on a
-- fresh database (already included in the inline CREATE TABLE check
-- above); required here for a database migrated before this stage
-- existed.
ALTER TABLE merchants DROP CONSTRAINT IF EXISTS merchants_invoice_awaiting_stage_check;
ALTER TABLE merchants ADD CONSTRAINT merchants_invoice_awaiting_stage_check
    CHECK (invoice_awaiting_stage IN ('AWAITING_NAME', 'AWAITING_PHONE', 'ITEMS', 'CONFIRM'));

-- ---------------------------------------------------------------------------
-- Migration: multi-currency support. Kika no longer assumes every
-- merchant is Nigerian — a merchant's default currency is now resolved
-- ONCE, from their WhatsApp number's country calling code, at signup
-- (see queries.findOrCreateMerchantByWhatsappNumber and
-- src/config/countryCurrency.js's offline calling-code lookup table; no
-- exchange-rate API involved). default_currency on merchants and
-- currency on ledger_entries already existed as columns before this
-- migration — they just always held the hardcoded 'NGN' default until
-- now, since nothing ever set them explicitly. This block only widens
-- default_currency to fit any real ISO 4217 code (it already fits 3
-- chars, so this is a no-op in practice) and documents the change; no
-- new columns are introduced.
-- ---------------------------------------------------------------------------
ALTER TABLE merchants ALTER COLUMN default_currency TYPE VARCHAR(3);
