...
function buildDigestSvg({ moneyInflowLabel, growthLabel, outstandingLabel, tradeDays, topDebtorName, topDebtorLabel }) {
  ...
  // --- Footer ---
  // The "Generated by AI" / "View Full Report" text and chevron that used
  // to sit here have been removed — deliberately silently: the divider
  // above and the vertical space/card height they used to occupy are
  // left exactly as they were, so nothing else about the card's size or
  // layout shifts. This just leaves that strip blank.
  const footerY = y;
  y = footerY + px(16);
...
<svg width="${CARD_WIDTH}" height="${cardHeight}" viewBox="0 0 ${CARD_WIDTH} ${cardHeight}" xmlns="http://www.w3.org/2000/svg">
  <rect x="0" y="0" width="${CARD_WIDTH}" height="${cardHeight}" fill="${COLORS.cardBg}" stroke="${COLORS.cardBorder}" stroke-width="1" />
  ${header}
